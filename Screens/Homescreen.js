...
export default function HomeScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const load = async () => {
        try {
          const saved = await AsyncStorage.getItem("bitzaChildProfile");
  ...
  const tools = [
    ...
    { title: "Calm", text: "Journal", icon: require("../assets/icons/support-calm-journal.png"), screen: "CalmJournal", color: "#EEF7E9" },
  ...
  const goToScreen = (screenName, params) => navigation.getParent()?.getParent()?.navigate(screenName, params) ?? navigation.navigate(screenName, params);
  const goToAddActivity = () => navigation.getParent()?.getParent()?.navigate("AddRoutineActivity") ?? navigation.navigate("AddRoutineActivity");
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.safeAreaTransparent}>
        ...
        {/* Top Row — icons aligned right */}
        <View style={styles.topRow}>
          <View style={styles.topIcons}>
            <TouchableOpacity activeOpacity={0.7} onPress={() => goToScreen("Notifications")} style={styles.bellWrapper}>
          ...
          <View style={styles.suggestCard}>
            ...
            {suggestions.map((s, i) => (
              <TouchableOpacity
                ...
                onPress={() => goToScreen(s.screen)}
                activeOpacity={0.8}
              >
                <View style={[styles.suggestToolIcon, { backgroundColor: s.color }]}>
                  <Image source={s.icon} style={{ width: 26, height: 26, resizeMode: "contain" }} />
                </View>
                <View style={styles.suggestTextWrap}>
                  <Text style={styles.suggestToolTitle}>{s.title}</Text>
        ...
        {/* Support Card */}
        <TouchableOpacity activeOpacity={0.8} style={styles.supportCard} onPress={() => navigation.getParent()?.getParent()?.navigate("SupportMode")}>
...
const styles = StyleSheet.create({
  ...
  // Top row — icons pushed to the RIGHT
  ...
  card: { backgroundColor: CARD, borderRadius: 16, borderWidth: 1, borderColor: BORDER, padding: 10, marginBottom: 8, shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 5, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  ...
  supportCard: { backgroundColor: "#FFDAD3", borderColor: "#FFC4BC", borderWidth: 1, borderRadius: 16, padding: 9, marginBottom: 10, flexDirection: "row", alignItems: "center" },
